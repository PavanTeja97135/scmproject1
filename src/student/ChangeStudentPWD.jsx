...
export default function ChangeStudentPwd() {
    ...
    useEffect(() => {
      const storedStudentData = localStorage.getItem('admin');
  ...
  const handleSubmit = async (e) => {
    ...
    try 
    {
      const response = await axios.put(`${config.url}/changestudentpwd`, {...formData,"username":studentData.username});
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit}>
         <div>
          <label className="white-label">Old Password</label>
          <input type="password" id="oldpassword" value={formData.oldpassword} onChange={handleChange} required />
        </div>
        <div>
          <label className="white-label">New Password</label>
